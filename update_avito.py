...
${badgeHtml}
  ...
  const completedTabCounter = document.querySelector('[data-marker="extended_profile_tabs/tab(closed)"] .styles-module-counter-uAip7');
  if (completedTabCounter) completedTabCounter.innerText = dbCompletedCount + 12;
...
if old_logic in html:
    ...
    with open('clean_avito (1).html', 'w', encoding='utf-8') as f:
        f.write(html)
    print("clean_avito updated successfully.")
else:
    print("Error: Could not find old_logic in clean_avito (1).html")

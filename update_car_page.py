...
# Update car_page.html script
with open('car_page.html', 'r', encoding='utf-8') as f:
    content = f.read()

# First, strip the old script from the file so we can append a clean one
content = re.sub(r'<!-- Supabase JS -->.*</body>', '</body>', content, flags=re.DOTALL)

# Inject CSS to hide body initially
if '<style id="dynamic-hide">' not in content:
    content = content.replace('</head>', '<style id="dynamic-hide">body { opacity: 0 !important; transition: opacity 0.3s ease-in; }</style></head>')
...
print("Updated car_page.html to hide content until dynamically loaded.")
